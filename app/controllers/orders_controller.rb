class OrdersController < ApplicationController
  ...
  def new
  	...
  	#add the basket stuff on this order
  	# overwrite the new order with an updated one with the basket stuff
  ...
  def build_order(o)
  	...
  	basket.each do |basket_item|
  		#each basket_item is a product_id
  		#lets find that product using the product id
